...
export function TypeTable({
  ...
  return (
    <div
      id={id}
      className={cn(
        '@container flex flex-col p-1 bg-fd-card text-fd-card-foreground rounded-2xl border my-6 text-sm overflow-hidden bg-muted/40',
        className,
      )}
      {...props}
    >
...
function Item({
  ...
  const [open, setOpen] = useState(() => {
    if (typeof window === 'undefined' || !parentId) return false;
    return window.location.hash === `#${parentId}-${name}`;
  });
